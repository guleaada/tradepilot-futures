...
1. Pulls real market data (spot klines for indicators — same geo-block failover as the spot bot — plus the futures funding rate).
...
3. Runs the deterministic rule engine — the only component that ever places orders:
   ...
   - **Stops/targets, symmetric**: stop = entry ∓ 1.5·ATR, TP = entry ± 2.5·ATR (sign by direction). Breakeven at +1.5R favorable, 50% partial at +2R, runner to +4R. Exits on stop, TP, or an opposite-direction regime flip with confidence ≥ 70.
