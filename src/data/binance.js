...
// Funding rate from the futures premiumIndex endpoint. Degrade gracefully:
// return null on any failure. Funding is applied against open-position P&L in
// the portfolio module, so a null here just means "no funding charged this
// cycle" (and the AI prompt notes it as unavailable).
...
const MOCK_PHASE = { BTCUSDT: 0, ETHUSDT: 2.8, SOLUSDT: 3.1, BNBUSDT: 4.7, XRPUSDT: 0.8 };
...
const MOCK_TREND = { BTCUSDT: 1, ETHUSDT: -1, SOLUSDT: 1, BNBUSDT: -1, XRPUSDT: 1 };
...
function mockTicker(pair) {
  ...
  // Mock quote volumes put SOL/XRP below the liquidity threshold on purpose,
  // so the PAIR_EXCLUDED path is exercised in demos. BTC (long trend) and
  // ETH/BNB (short trend) survive the filter.
  return {
    lastPrice,
    priceChangePercent: mockTrend(pair) * 2.4,
    volume: 24000,
    quoteVolume: lastPrice * 24000,
  };
}
